/*
    ...
    Example Explanation
        ...
        But you need B[0] = 2 gas to travel to station 1. If you start from index 1, you can fill in A[1] = 2 amount of gas. 
        Now your tank has 2 units of gas. You need B[1] = 1 gas to get to station 0. So, you travel to station 0 and still have 1 unit of gas left over. 
...
int Solution::canCompleteCircuit(const vector<int> &A, const vector<int> &B) {
    ...
    for(int i=0;i<N;i++){
        ...
        if(gas_current <0){
            start =i+1;
            gas_deficient += gas_current;
            gas_current =0;
        }
    }
    if(gas_current + gas_deficient >=0){
        return start;
    }
    return -1;
}
